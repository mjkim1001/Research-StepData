...
#fig3.1
#function thick_draw is defined below
par(mfrow = c(1,1))
...
id=3
par(mfrow=c(2,2),mar = c(2, 2, 4, 2))
thick_draw(step[,selected[2]], 30, sqBound_es, time=TRUE, smooth = 0, smooth.num = 5,zeroBound = T,xl = "(a)",ymin=-20,ymax=160,col=rgb(0,0,1,0.3))
thick_draw(step[,selected[id]], 30, sqBound_es, time=TRUE, smooth = 0, smooth.num = 5,zeroBound = T,xl = "(b)",ymin=-20,ymax=160,col=rgb(1,0,0,0.2))
plot(time,step[,selected[1]],type='n',main="(c)",ylab="", ylim=c(-20,160), xlab='')
...
plot( time, pmin(sqBound_es((step[,selected[2]]), 30, max),
                 sqBound_es((step[,selected[id]]), 30, max))/pmax(sqBound_es((step[,selected[2]]), 30, max),sqBound_es((step[,selected[id]]), 30, max)),      type='l' ,main="(d)", ylab="" , ylim=c(0,1), xlab='')
